...
- **Models exposed by the active full-access virtual key (validated 2026-05-28):** GPT-5 family + Claude Opus 4.5/4.6 + Gemini 2.5 Pro/Flash + Gemini image gen. NO Claude Sonnet on any current key (the path is to request a new virtual key with Sonnet access if needed).
